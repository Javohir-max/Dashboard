...
const groupedByDate = computed(() => {
    // Фильтруем данные
    const filteredData = hotelData.value.filter(fl => 
...
const getAdvertising = async () => {
    try {
        const response = await axios.get('http://localhost:3000/advertising?cotegory=hotel')
...
<template>
    <div class="hotel">
        <CreateAdvertisements v-if="hotelData.length === 0" />
